...
test.cb('When a page object is included in the location state, should not fetch JSON of the new page on history changes', t => {
  t.plan(4)
...
test.cb('When a page object is not included in the location state, and the fetched page redirects, the URL should match the redirect', t => {
  t.plan(3)
  ...
  nock(host)
    .get('/follow')
    .reply(200, { page: 'FollowPage', props: {} })

  store.subscribe(function () {
    const [action] = store.getActions()

    t.is(SET_LOCATION, action.type)
    t.is('/follow', action.location.pathname)
    t.is('FollowPage', action.page)
    t.end()
  })

  history.push({ pathname: '/redirects' })
})
